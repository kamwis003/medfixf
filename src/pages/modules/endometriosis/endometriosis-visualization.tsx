...
const PAIN_COLORS = [
  { min: 0, max: 1, color: 'bg-green-300 dark:bg-green-700', label: t('endometriosis.visualization.pain.01') },
  { min: 2, max: 3, color: 'bg-yellow-200 dark:bg-yellow-700', label: t('endometriosis.visualization.pain.23') },
  { min: 4, max: 6, color: 'bg-orange-300 dark:bg-orange-700', label: t('endometriosis.visualization.pain.46') },
  { min: 7, max: 8, color: 'bg-rose-300 dark:bg-rose-700', label: t('endometriosis.visualization.pain.78') },
  { min: 9, max: 10, color: 'bg-red-400 dark:bg-red-800', label: t('endometriosis.visualization.pain.910') },
]
...
const WEEKDAYS: string[] = t('endometriosis.visualization.weekdays', { returnObjects: true }) as string[]
  ...
  useEffect(() => {
    ...
    const fetchEntries = async () => {
      ...
      try {
        ...
        const points: DiaryEntryPoint[] = (res.data ?? [])
          .map((e) => ({ date: e.date, painLevel: Number(e.painLevel ?? 0) }))
    ...
    <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          {t('endometriosis.visualization.subtitle')}
        </p>
      </div> 
      <Card>
        <CardHeader>
        ...
        <CardContent>
          ...
          {isLoading ? (
            <div className="py-8 text-center text-muted-foreground">{t('endometriosis.loading')}</div>
          ) : (
            <>
                  ...
                  const cell = (
                    <div
                      ...
                      className={[
                        ...
                        painLevel !== undefined ? getPainColor(Number(painLevel)) : 'hover:bg-muted',
                      ...
                      <TooltipProvider key={key} delayDuration={200}>
                        <Tooltip>
                          ...
                          <TooltipContent side="top" className="max-w-[200px]">
                            <span className="font-semibold">{t('endometriosis.diary.painlevel')}:</span> {painLevel ?? '-'}<br />
